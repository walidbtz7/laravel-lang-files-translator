...
{
    protected $signature = 'translate:lang {from : translate from language} {to : translate to language} {--prefix= : static prefix path}';
    ...
    public function handle(TranslateService $translateService)
    {
        ...
        // Retrieve the static prefix from the command argument
        $staticPrefix = $this->option('prefix');
        
        // Pass the static prefix along with the other arguments to the TranslateService
        $translateService->to($this->argument('to'))->from($this->argument('from'))->setStaticPrefix($staticPrefix)->translate();
